...
class LivelinessCaptureFragment :
    BaseFragment<VerifyDocumentViewModel, FragmentLivelinessCaptureBinding, AppRepository>() {
    ...
    private var isIndentStarted = false;
            ...
            if (livelinessCameraXResult.error == null) {
                val listOfImages = arrayListOf<ByteArray>().apply {
                    livelinessCameraXResult.createdBySteps?.let { photoResultList ->
                        this.addAll(photoResultList.map {
                            Base64.decode(
                                it.fileBase64,
                                Base64.NO_WRAP
                            )
                ...
                livelinessCameraXResult.error?.let {
                    Timber.tag("livelinessCameraXResult").e(it.toString())
                }
            }
        }
    }
